...
class ShapeDetector(Node):
    def __init__(self):
        ...
        self.DOCK_DETECTION_TOLERANCE = 0.1
        self.dock_procedure_done = False

        self.IGNORE_ZONES = [
            (-1.1, -5.2, -0.3, -4.4), (-2.45, -5.2, -1.95, -4.4),
            (0.7, -2.6, 1.1, -2.0), (-2.4, -0.5, -1.7, 0.5),
            (-1.1, -0.5, -0.3, 0.5), (-3.1, -7.5, 1.4, -6.0),
            (0.7, 1.1, 1.2, 1.7), (-3.1, 1.9, 1.4, 2.4),
        ]
        ...
        # --- NEW State Machine Variables for Delayed Publishing ---
        self.state = "IDLE" # Can be IDLE, WAITING_DELAY, PUBLISHING
        self.pending_detection_shape = None
        self.delay_start_time = None
        self.square_delay = 3.0
        self.triangle_delay = 0
        ...
        # Use a timer to handle state transitions and publishing reliably
        self.timer = self.create_timer(0.1, self.state_machine_tick)

        self.get_logger().info('Final Shape & Dock Detector with Delayed Publishing is running.')
        # cv2.namedWindow("LiDAR View", cv2.WINDOW_NORMAL)
        # cv2.namedWindow("Dilated View", cv2.WINDOW_NORMAL)
    ...
    def odom_callback(self, msg):
        ...
        # Dock Check remains in odom_callback
        if self.current_pose and not self.dock_procedure_done and self.state == "IDLE":
            ...
            if dist_to_dock < self.DOCK_DETECTION_TOLERANCE:
                ...
                self.state = "PUBLISHING" # Enter publishing state directly for dock
    ...
    def scan_callback(self, msg):
        # Scan callback only does detection now, state machine handles delays/publishing
        current_time = self.get_clock().now().nanoseconds / 1e9

        # Only detect if IDLE and not cooling down
        ...
        try:
            ...
            for cnt in contours:
                ...
                if shape:
                    ...
                    if not self._is_shape_in_ignore_zone(shape_global_x, shape_global_y):
                        detected_shapes.append({"shape": shape, "local_pos": (center_x_local, center_y_local)})
            
            # --- Buffering and Decision Logic ---
            ...
            if current_square:
                ...
                self.get_logger().warn(f"Detected SQUARE.")
            elif current_triangle:
                self.triangle_buffer = {'timestamp': time.time()} # Use system time
            ...
            if not current_triangle and self.triangle_buffer:
                if time.time() - self.triangle_buffer['timestamp'] > self.triangle_buffer_timeout:
                    ...
                    self.get_logger().warn(f"Confirmed TRIANGLE from buffer.")
                    self.triangle_buffer = None
            
            # --- If a shape is accepted, trigger the delay state ---
            if accepted_shape:
                self.get_logger().info(f"Accepted {accepted_shape}. Pausing nav and starting delay.")
                self.state = "WAITING_DELAY"
                self.pending_detection_shape = accepted_shape
                self.delay_start_time = self.get_clock().now()
                self.last_detection_time = current_time # Start cooldown after initial detection

            # Visualization can be added back here if needed
            # cv2.imshow("LiDAR View", vis_img)
            # cv2.imshow("Dilated View", dilated_img)
            # cv2.waitKey(1)
    ...
    def state_machine_tick(self):
        """Handles state transitions for delayed publishing."""
        ...
        if self.state == "WAITING_DELAY":
            ...
            if (now - self.delay_start_time).nanoseconds > (delay_duration * 1e9):
                ...
                # Get the ROBOT'S current pose AFTER the delay
                if self.current_pose:
                    ...
                    self.get_logger().error("Cannot publish status, current pose is unknown.")
                    self.state = "IDLE" # Reset state on error
                    self.pause_pub.publish(Bool(data=False)) # Resume nav on error

        # Note: The PUBLISHING state is managed within _publish_status now
    ...
    def _filter_close_vertices(self, vertices):
        ...
        # Final check: is the last point close to the first point?
        if len(filtered) > 2:
    ...
    def _is_shape_in_ignore_zone(self, shape_x, shape_y):
    ...
    def _publish_status(self, status, x, y, is_dock=False):
        ...
        # Ensure resume message is sent reliably
        resume_msg = Bool(data=False)
        for _ in range(5):
             self.pause_pub.publish(resume_msg)
             time.sleep(0.02)
             
        # Reset state machine AFTER publishing is complete
        self.state = "IDLE"
        self.pending_detection_shape = None
        self.delay_start_time = None
        # Reset dock detection flag only if it wasn't the dock procedure
        if not is_dock:
            # Re-enable dock detection if we've moved away sufficiently
            if self.current_pose:
                 dist_to_dock = math.sqrt((self.current_pose[0] - self.DOCK_STATION_COORDS[0])**2 + (self.current_pose[1] - self.DOCK_STATION_COORDS[1])**2)
                 if dist_to_dock > (self.DOCK_DETECTION_TOLERANCE + 0.5): # Reset if moved 0.5m away
                      self.dock_procedure_done = False


    # handle_dock_detection is now folded into odom_callback and _publish_status
...
def main(args=None):
    ...
    try: rclpy.spin(detector_node)
        ...
        if 'detector_node' in locals() and rclpy.ok() and detector_node.context.ok():
